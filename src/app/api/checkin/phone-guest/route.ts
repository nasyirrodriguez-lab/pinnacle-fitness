...
// Public guest check-in endpoint used by visitors scanning the kiosk QR
// on their own phone. No kiosk-device cookie required — same trust model
// as walking through the door. The selfie is required, so we still have
// a face in the visits log.
const fieldsSchema = z.object({
  ...
  // Everyone checking in accepts the T&Cs — required, not optional.
...
export async function POST(request: NextRequest) {
  ...
  const { data: visit, error } = await admin
    ...
    .single()
  if (error || !visit) {
    console.error('[checkin/phone-guest] insert failed:', error)
