  ...
  return (
    <div className="min-h-screen bg-[#E8E4DC]">
        ...
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              ...
              className="text-xs text-[#6B6560] hover:text-[#3A3733] transition"
            >
              Billing
            </Link>
            {member.role === 'admin' && (
              <Link
                href="/admin"
                className="text-xs text-[#3A3733] hover:text-[#2a2825] font-medium border border-[#3A3733]/30 px-3 py-1.5 rounded-lg hover:bg-[#3A3733]/5 transition"
              >
                Admin
              </Link>
            )}
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        ...
        {/* Welcome */}
        <div className="mb-8">
          <h1 className="font-serif text-3xl text-[#3A3733]">
            Welcome back, <span className="text-[#3A3733]">{firstName}</span>
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Member Card */}
          <div className="lg:col-span-2 bg-white border border-[#CCC8C0] rounded-2xl p-6">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h2 className="font-serif text-xl text-[#3A3733]">{fullName}</h2>
                <p className="text-[#6B6560] text-sm mt-0.5">{user.email}</p>
              </div>
              <span className={`px-3 py-1 rounded-full text-xs font-medium uppercase tracking-wide ${planColors[member.plan_type ?? ''] ?? 'bg-[#CCC8C0] text-[#6B6560]'}`}>
                {member.plan_type ? (planNames[member.plan_type] ?? member.plan_type) : 'No plan selected'}
              </span>
            </div>

            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-[#E8E4DC] rounded-xl p-4">
                <p className="text-xs text-[#6B6560] uppercase tracking-[0.1em] mb-1">Member Since</p>
                <p className="text-[#3A3733] font-medium text-sm">{memberSince}</p>
              </div>
              <div className="bg-[#E8E4DC] rounded-xl p-4">
                <p className="text-xs text-[#6B6560] uppercase tracking-[0.1em] mb-1">Member ID</p>
                <p className="text-[#3A3733] font-mono font-semibold text-sm">{member.member_code}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2">
                {(planPerks[member.plan_type] || []).map((perk) => (
                  <li key={perk} className="flex items-center gap-2 text-sm text-[#3A3733]">
                    <span className="w-4 h-4 rounded-full bg-[#3A3733]/10 border border-[#3A3733]/30 flex items-center justify-center flex-shrink-0">
                      <svg className="w-2.5 h-2.5 text-[#3A3733]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
          ...
          {/* QR Code Card */}
          <div className="bg-white border border-[#CCC8C0] rounded-2xl p-6 flex flex-col items-center justify-center">
        ...
        <div className="mt-6 grid grid-cols-3 gap-4">
          {[
            ...
            <div key={label} className="bg-white border border-[#CCC8C0] rounded-xl p-4 text-center">
        ...
        <div className="mt-4 flex gap-3 flex-wrap">
          <Link
            ...
            className="flex items-center gap-1.5 text-xs text-[#6B6560] hover:text-[#3A3733] transition border border-[#CCC8C0] rounded-lg px-3 py-2 hover:bg-white"
          >
