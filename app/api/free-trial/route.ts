...
export async function POST(req: NextRequest) {
  const { name, email, phone, classId, className, classDay, classTime } = await req.json()
  ...
  const adminHtml = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; color: #1C1A17;">
      ...
      <table style="width: 100%; border-collapse: collapse;">
        <tr><td style="padding: 8px 0; border-bottom: 1px solid #E8E3D9; color: #6B6560; width: 100px;">Name</td><td style="padding: 8px 0; border-bottom: 1px solid #E8E3D9;">${name}</td></tr>
        <tr><td style="padding: 8px 0; border-bottom: 1px solid #E8E3D9; color: #6B6560;">Email</td><td style="padding: 8px 0; border-bottom: 1px solid #E8E3D9;"><a href="mailto:${email}">${email}</a></td></tr>
        <tr><td style="padding: 8px 0; border-bottom: 1px solid #E8E3D9; color: #6B6560;">Phone</td><td style="padding: 8px 0; border-bottom: 1px solid #E8E3D9;">${phone}</td></tr>
        <tr><td style="padding: 8px 0; border-bottom: 1px solid #E8E3D9; color: #6B6560;">Class</td><td style="padding: 8px 0; border-bottom: 1px solid #E8E3D9;">${className}</td></tr>
        <tr><td style="padding: 8px 0; color: #6B6560;">When</td><td style="padding: 8px 0;">${classDay} at ${formattedTime}</td></tr>
      </table>
    </div>
  `
  ...
  await Promise.all([
    resend.emails.send({
      ...
      to: email,
      subject: `You're booked for ${className} at Pinnacle Fitness!`,
      html: confirmationHtml,
    }),
    resend.emails.send({
      from: 'Pinnacle Fitness <onboarding@resend.dev>',
      to: 'nasyirrodriguez@gmail.com',
      subject: `New Free Trial Booking: ${name} → ${className}`,
      html: adminHtml,
    }),
  ])
